...
export const AllUsers = () => {
  ...
  const handleBlock = (userId,currentStatus) => {
    ...
    axios.patch(`http://localhost:3000/users/${userId}`, {is_blocked:newStatus })
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          {users.map(item=>(
             ...
             <td className="px-6 py-4">
                <Link to={`${item.id}`}>
                  <button className="px-3 py-2 text-sm font-medium text-white bg-blue-500 rounded-lg shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50">
                    See More
                  </button>
                </Link>
              </td>
